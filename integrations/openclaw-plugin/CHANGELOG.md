...
- **The documented `namespace` config field had no effect.** `openclaw.plugin.json`'s `configSchema` and every README/guide example documented `namespace` as an isolation boundary, and the shared client accepted and defaulted it, but `PensyveClient` never read it and never sent it in any request — `pensyve-mcp-gateway`'s REST API (`RecallRequest`, `RememberRequest`, etc.) has no per-request namespace parameter at all; the gateway derives an isolated namespace purely from the authenticated tenant (API key in cloud mode, one shared default namespace in local/dev mode). A client-supplied `namespace` string could never have isolated anything, so documenting and accepting it was misleading rather than merely unimplemented. Removed the field from `openclaw.plugin.json`'s `configSchema`, `PensyveConfig`/`resolveConfig()` in `integrations/shared/pensyve-client.ts`, and the guide/README examples, rather than leave it as a silent no-op. The MCP path's `PENSYVE_NAMESPACE` env var (read by `pensyve-mcp` itself, not the gateway's REST API) is unaffected and remains the real namespace-isolation mechanism for that path.
