...
- **off**: Do nothing. Inform the user: "Context loading is disabled. Use `pensyve_recall` to search memories on demand."
- **summary**: Load a concise overview (10-15 lines max). This is the default.
...
>
> _Use `pensyve_recall` to search for specific memories._
...
>
> _Total memories loaded: 11 | Use `pensyve_recall` for targeted search | `pensyve_inspect` for entity details_
...
- If no memories are found at all, say: "No memories found. This appears to be a fresh session. Use `pensyve_remember` to start building context."
...
- If the MCP server is not connected, inform the user: "Pensyve MCP server is not connected. Context loading skipped. Verify your `PENSYVE_API_KEY` environment variable and MCP configuration."
