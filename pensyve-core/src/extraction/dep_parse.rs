...
/// Domain-specific namespace UUID for `kg_entities` / relation granules.
///
/// Used as the namespace argument to [`Uuid::new_v5`] so the resulting
/// granule UUIDs are stable across Rust toolchain versions (RFC 4122
/// v5 is SHA-1-based and explicitly defined to be reproducible — unlike
/// `std::hash::DefaultHasher`, whose algorithm "is not specified, and
/// so its hashes should not be relied upon over releases" per the
/// stdlib docs).
///
/// The constant itself is a random v4 UUID minted once for the
/// `pensyve::extraction::dep_parse` domain — namespacing the v5 hash
/// keeps our granule IDs in their own keyspace so a future caller can
/// use `Uuid::new_v5` against a different namespace without colliding
/// with us. Do NOT change this constant after release: every stored
/// `entity_index` / `relation_index` row is keyed off it.
///
/// `CodeRabbit` + `claude-bot` PR #115 P0 #3 review.
const KG_GRANULE_NAMESPACE: Uuid = Uuid::from_u128(0x4f5d6d8c_9e3a_4b1f_a2c5_7e1d9c3f0a82);

/// Derive a stable `Uuid` from a granule key (entity lemma or
/// `"subject predicate object"` triple string) via RFC 4122 v5.
///
/// Stability guarantee: `granule_uuid(k)` returns the same `Uuid` for
/// the same `k` across every Rust toolchain version, every machine,
/// and every Pensyve build — that is the entire reason this function
/// exists. It is NOT a cryptographic identifier.
fn granule_uuid(key: &str) -> Uuid {
    Uuid::new_v5(&KG_GRANULE_NAMESPACE, key.as_bytes())
}
...
mod tests {
    ...
    #[test]
    fn granule_uuid_is_deterministic_within_a_run() {
        // Same key → same Uuid. Trivial sanity check but also the
        // baseline invariant the cross-toolchain stability case rests
        // on (idempotent within a process implies idempotent across
        // processes for a hash function whose algorithm is fixed).
        assert_eq!(granule_uuid("Alice"), granule_uuid("Alice"));
        assert_ne!(granule_uuid("Alice"), granule_uuid("Acme"));
    }

    #[test]
    fn granule_uuid_v5_stability_pin() {
        // RFC 4122 v5 hashes are SHA-1 based — the output is fixed
        // forever given a `(namespace, name)` pair. We pin two
        // expected values so a future change to either
        // `KG_GRANULE_NAMESPACE` or the hash construction trips this
        // test loudly. Re-keying the namespace at any point would
        // invalidate every stored `kg_entities` row, so this guard is
        // load-bearing.
        //
        // Generated by `Uuid::new_v5(&KG_GRANULE_NAMESPACE, b"Alice")`
        // on the initial namespace constant. Do NOT update these
        // strings without coordinating a migration that re-keys
        // existing stored granule UUIDs.
        assert_eq!(
            granule_uuid("Alice").to_string(),
            "e839cd65-e8e5-549e-86d7-c9f941817d02"
        );
        assert_eq!(
            granule_uuid("Alice works_at Acme").to_string(),
            "efb3dfaf-528f-50fb-a0db-55e434842895"
        );
    }
