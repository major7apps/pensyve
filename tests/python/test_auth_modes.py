...
def test_required_mode_exits_without_keys():
    with patch.dict(os.environ, {"PENSYVE_AUTH_MODE": "required", "PENSYVE_API_KEYS": ""}, clear=False):
...
def test_required_mode_starts_with_keys():
    with patch.dict(os.environ, {"PENSYVE_AUTH_MODE": "required", "PENSYVE_API_KEYS": "test-key-123"}, clear=False):
        import importlib
...
def test_disabled_mode_skips_auth():
    with patch.dict(os.environ, {"PENSYVE_AUTH_MODE": "disabled", "PENSYVE_API_KEYS": ""}, clear=False):
        import importlib
